...
If you want to deploy on your VPS, please clone the [Frontend](https://github.com/U1in/AssassinGo-Front-End) and modify the `base_url` of AJAX and WebSocket, then run `npm build` and copy the output to `web/` dirctionary as [deploy.sh](./deploy.sh) says. 
